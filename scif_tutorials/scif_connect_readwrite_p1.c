...
/* scif_connect_readwrite_p1.c : this program demonstrates scif_readfrom & scif_writeto APIs
...
int write_data(scif_epd_t epd, win_t *buffer, int msg_size, int use_cpu, char *msg)
{
	...
	if ((err = scif_writeto(epd,
		...
		/* errno ENXIO implies : No such device or address, which indicates
		 * a bad offset/absense of a registered window. So, node sends a
		 * msg to peer requesting to register a window
		 */
